...
document.getElementById("status").addEventListener("mousedown", function(){
    if (! audioContext) {
        try {
        ...
        // omit the pitchVectorCallback if all you need is the pitch value, and not the activation of all pitches
        featureGraph(audioContext, pitchCallback, pitchVectorCallback);
    }
});

let confidencetext = document.getElementById('voicing-confidence');
let pitchvaluetext = document.getElementById('estimated-pitch');
const pitchCallback=function(pc) {
  if (pc.confidence > .5){
    pitchvaluetext.innerHTML = pc.pitch;
  }
  else {
...
// ----   Below is all just for getting and plotting pitch activation across the spectrum -----------

// a function that accepts the activation vector for each frame
const pitchVectorCallback = (function() {
    ...
    for (var i = 0; i < inferno.length; i++) {
      temparray = new Uint8ClampedArray(4);
      temparray.set(inferno[i]);
      inferno[i] = temparray;
    }
